---
title: Privacy Guides
date:
---

# Privacy Guides

Welcome to the Privacy Guides collection. These guides cover privacy-focused configurations across hardware, virtualization, networking, and software.

> [!info] About These Guides
> These are living documents, regularly updated as tools and best practices evolve. Each guide prioritizes practical implementation with security-first principles.

## Guide Categories

### Hardware

Secure hardware configuration and privacy-focused operating systems.

- [[AsusWRT-Merlin Hardened Router Install Guide]]
- [[GrapheneOS Install Guide]]

### Virtualization

Isolated environments for testing and secure computing.

- [[VirtualBox Linux Mint Setup Guide]]

### WiFi & Networking

Platform-specific WiFi hardening and network privacy configurations.

- [[Linux WiFi Privacy Guide]]
- [[macOS WiFi Privacy Guide]]
- [[Windows WiFi Privacy Guide]]

### General Privacy

Cross-platform privacy frameworks and implementation strategies.

- [[Adaptive Privacy Framework - A Modular Approach]]
- [[Browser Hardening]]
- [[Privacy Implementation Guide]]

---

> [!warning] Disclaimer
> These guides are provided for educational purposes. Always test configurations in a safe environment before deploying to production systems. The author assumes no liability for misconfigurations or unintended consequences. 

>[!note]I am in no way associated or receive incentives from any of the hardware, software, or individuals referenced here. These are my notes from my experience compiled with LLM as I have gathered them over time. I have found these to be the most useful setups that work for me. If you have any feedback or any notes on any of the documentation, please feel free to reach out to drew@harbingersec.tech. Thanks for reading :)

> [!tip] Contributing
> If you have found any of these guides to be useful, please leave me a tip - 
>
> [![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/Z8Z010XFND)
>
> Found an issue or have improvements? These guides are open source under CC BY-SA 4.0.

---

**Last Updated**: January 2025  
**License**: [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)
EOF